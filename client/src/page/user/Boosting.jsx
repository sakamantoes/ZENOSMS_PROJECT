...
const PLATFORM_CONFIG = {
  ...
  tiktok: {
    Icon: Music,
    gradient: "from-purple-500/20 to-gray-900/40",
...
const FALLBACK_CONFIG = {
  ...
  selectedBorder: "border-gray-400/60",
  text: "text-gray-400",
  bg: "bg-gray-500/10",
...
const cfg = (platform) =>
  PLATFORM_CONFIG[platform?.toLowerCase()] ?? FALLBACK_CONFIG;
...
const OrderModal = ({
  ...
  const totalCost =
    Number.isFinite(qty) && qty > 0
      ? (qty / 1000) * service.sellingPrice
      : 0;
  const canAfford = balance >= totalCost && totalCost > 0;
  const qtyValid =
    Number.isFinite(qty) && qty >= service.min && qty <= service.max;

  const handleConfirm = () => {
    if (!link.trim()) {
      setLinkError("Please enter the target URL.");
      return;
    }
    setLinkError("");
      ...
      >
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`w-full pl-9 pr-4 py-2.5 rounded-xl bg-white/5 border text-white text-sm placeholder-gray-500 outline-none transition-colors focus:border-emerald-500/50 ${
                linkError ? "border-red-500/40" : "border-white/10"
              }`}
            />
          </div>
          {linkError && (
        ...
        <div className="mb-4">
          ...
          <div className="relative">
            ...
            <input
              ...
              className={`w-full pl-9 pr-4 py-2.5 rounded-xl bg-white/5 border text-white text-sm outline-none transition-colors focus:border-emerald-500/50 ${
                quantity && !qtyValid ? "border-red-500/40" : "border-white/10"
              }`}
            />
          </div>
          {quantity && !qtyValid && (
            <p className="mt-1 text-xs text-red-400">
              Must be between {service.min.toLocaleString()} and{" "}
              {service.max.toLocaleString()}.
            </p>
          )}
        </div>
        ...
        <div className="space-y-2 mb-4">
          <div className="flex justify-between items-center p-3 rounded-xl bg-white/5 border border-white/5">
            ...
            <span className="text-sm font-medium text-white">
              {formatNaira(service.sellingPrice)} / 1,000
            </span>
          </div>
          <div className="flex justify-between items-center p-3 rounded-xl bg-white/5 border border-white/5">
            ...
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                ...
                <p className="text-xs text-gray-400 mt-0.5">
                  You need {formatNaira(totalCost - balance)} more to place
                  this order.
                </p>
                <button
        ...
        <div className="flex gap-3">
          ...
          <button
            ...
            disabled={ordering || !canAfford || !qtyValid || !link.trim()}
  ...
  >
    ...
    <div className="mb-3">
      <p className="text-xl font-bold text-emerald-400">
        {formatNaira(service.sellingPrice)}
      </p>
    ...
    <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs text-gray-400 mb-3">
      <span>
        Min:{" "}
        <span className="text-gray-300">{service.min.toLocaleString()}</span>
      </span>
      <span>
        Max:{" "}
        <span className="text-gray-300">{service.max.toLocaleString()}</span>
      </span>
    </div>
...
const Boosting = () => {
  ...
  const handleSelectCategory = async (category) => {
    ...
    try {
      const res = await getSocialServicesByCategory(
        selectedPlatform,
        category,
      );
  ...
  const handleConfirmOrder = async ({ link, quantity }) => {
    ...
    try {
      ...
      setSuccessMessage(
        "Order placed successfully! Your boost is being processed.",
      );
        ...
        >
          <div>
            ...
            <p className="text-sm text-gray-400 mt-1">
              Grow your social presence with real engagement
            </p>
          </div>
          <div className="flex items-center gap-3">
            ...
            >
              <RefreshCw
                className={`w-5 h-5 ${platformsLoading ? "animate-spin" : ""}`}
              />
            </button>
          </div>
        </motion.div>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-3">
              {platforms.map((platform) => {
                ...
                return (
                  ...
                  >
                    <div
                      className={`w-10 h-10 rounded-xl ${c.bg} flex items-center justify-center border ${c.border}`}
                    >
                      <c.Icon className={`w-5 h-5 ${c.text}`} />
                    </div>
                    <span
                      className={`text-sm font-semibold capitalize ${
                        isSelected ? c.text : "text-gray-300"
                      }`}
                    >
                      {platform}
                    </span>
                    {isSelected && (
                      <span
                        className={`text-[10px] font-medium rounded-full px-2 py-0.5 border ${c.bg} ${c.border} ${c.text}`}
                      >
                        Selected
                      </span>
                    )}
                ...
                <div className="flex flex-col items-center gap-3 rounded-xl border border-white/10 bg-white/5 p-10 text-center">
                  ...
                  <div>
                    ...
                    <p className="mt-1 text-xs text-gray-400">
                      No services found for this category right now.
                    </p>
                  </div>
                </div>
              )}
        ...
        <AnimatePresence>
          {orderService && (
            <OrderModal
              ...
              onDepositClick={handleDepositFromModal}
            />
          )}
        </AnimatePresence>
