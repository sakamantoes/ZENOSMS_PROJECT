...
const FormateHistory = () => {
  ...
  const fetchOrders = useCallback(async (isRefresh = false) => {
    ...
    try {
      const res = await getWorkingFormatHistory();
    ...
    return (
      <div className="min-h-screen bg-gradient-to-br from-gray-950 via-gray-900 to-black/95 flex items-center justify-center">
        <div className="text-center">
          ...
          <p className="text-gray-400">Loading format history...</p>
        </div>
      </div>
    );
  }
        ...
        >
          <div>
            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-emerald-500/10 border border-emerald-500/20 mb-2">
              ...
              <span className="text-xs font-medium text-emerald-400">Format History</span>
            </div>
            <h1 className="text-2xl md:text-3xl font-bold text-white font-['Space_Grotesk']">
              Format Purchases
            </h1>
            <p className="text-gray-400 text-sm mt-1">
              All formats you've bought from the working catalogue
            </p>
          </div>
          <button
        ...
        >
          {filtered.length === 0 ? (
            <div className="p-16 text-center">
              ...
              <p className="text-white font-semibold font-['Space_Grotesk']">No format orders found</p>
              <p className="text-gray-400 text-sm mt-1">
                {search || filterStatus !== 'all'
                  ? 'Try adjusting your filters'
                  : "You haven't purchased any formats yet"}
              </p>
            </div>
          ) : (
