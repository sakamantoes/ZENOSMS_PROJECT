...
import { getWorkingOrders } from "../../Service/admin.js";
...
const TYPE_STYLES = {
  tool: "border-[#07cf00]/20 bg-[#07cf00]/10 text-[#07cf00]",
  format: "border-violet-500/20 bg-violet-500/10 text-violet-400",
};
...
const WorkingOrderManagement = () => {
  ...
  const fetchOrders = async () => {
    ...
    setError("");
    try {
      const res = await getWorkingOrders();
      setOrders(res.data?.workingOrders ?? []);
      setStats(res.data?.stats ?? {});
    } catch (err) {
      setError(getErrorMessage(err, "Unable to fetch working orders."));
      setOrders([]);
    } finally {
      setLoading(false);
    }
      ...
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div className="flex items-start gap-3">
          <div className="flex h-10 w-10 shrink-0 items-center justify-center rounded-xl border border-[#07cf00]/20 bg-[#07cf00]/10">
            <ClipboardList size={18} className="text-[#07cf00]" />
          </div>
          <div>
            <h1 className="text-2xl font-bold text-white">Working Orders</h1>
            <p className="mt-1 text-sm text-gray-400">
              Review purchases of working photos, formats, and tools.
            </p>
          </div>
        </div>
      </div>
        ...
        <div className="flex flex-wrap gap-3">
          <div className="relative min-w-48 flex-1">
            ...
            <input
              ...
              placeholder="Search by product, order ref, or customer…"
                          ...
                          <div className="flex items-center gap-2.5">
                            {order.imageUrl ? (
                              ...
                              <div className="flex h-8 w-8 shrink-0 items-center justify-center rounded-lg border border-white/10 bg-white/5">
                                <Package2 size={13} className="text-gray-500" />
                              </div>
                            )}
          ...
          <div className="max-h-[90vh] w-full max-w-lg overflow-y-auto rounded-2xl border border-white/10 bg-[#0d0d0d] p-6 shadow-2xl">
            <div className="flex items-center justify-between gap-3">
              <div className="flex items-center gap-2.5">
                <div className="flex h-8 w-8 items-center justify-center rounded-lg border border-[#07cf00]/20 bg-[#07cf00]/10">
                  <ClipboardList size={15} className="text-[#07cf00]" />
                </div>
                <div>
            ...
            <div className="mt-5 space-y-4">
              ...
              <div className="rounded-lg border border-white/10 bg-white/5 p-3">
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="text-lg font-bold text-[#07cf00]">
                      {formatCurrency(selectedOrder.sellingPrice)}
                    </p>
                  </div>
                </div>
                {selectedOrder.linkUrl && (
                  <a
                    href={selectedOrder.linkUrl}
                    target="_blank"
                    rel="noreferrer"
                    className="mt-2 inline-block truncate text-xs text-[#07cf00] hover:underline"
                  >
                    {selectedOrder.linkUrl}
                  </a>
                )}
              ...
              <div className="grid grid-cols-2 gap-2.5">
                <div className="rounded-lg border border-white/10 bg-white/5 p-3">
                  <p className="text-[11px] text-gray-500">Stock Pics</p>
                  <p className="text-sm font-semibold text-white">
                    {selectedOrder.stockPics ?? "—"}
                  </p>
                </div>
                <div className="rounded-lg border border-white/10 bg-white/5 p-3">
                  <p className="text-[11px] text-gray-500">Stock Imgs</p>
                  <p className="text-sm font-semibold text-white">
                    {selectedOrder.stockImg ?? "—"}
                  </p>
                </div>
                <div className="rounded-lg border border-white/10 bg-white/5 p-3">
                  <p className="text-[11px] text-gray-500">Placed</p>
...
export default WorkingOrderManagement;
