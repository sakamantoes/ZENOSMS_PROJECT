...
const INITIAL_DETAIL = { selected: null, entries: [], loading: false, error: "" };
...
const AdminUsaServices = () => {
  ...
  const loadServiceNames = async () => {
    ...
    try {
      ...
      setList({ items: normalizeServiceNameResponse(res, "getatext"), loading: false, error: "" });
    } catch (err) {
      setList({ items: [], loading: false, error: getErrorMessage(err, "Unable to fetch USA services.") });
    }
  };
  ...
  const loadServiceEntries = async (service) => {
    ...
    try {
      const res = await getGetatextServiceNames({ page: 1, limit: 100, service: service.name });
      setDetail({
        selected: service,
      ...
      setDetail((s) => ({ ...s, loading: false, error: getErrorMessage(err, "Unable to fetch service entries.") }));
    }
  };

  useEffect(() => { loadServiceNames(); }, []);

  const handleToggle = async (service, active) => {
    setActivation((s) => ({ ...s, loadingId: service.id, errors: { ...s.errors, [service.id]: "" } }));
    try {
      ...
      setList((s) => ({
        ...
        items: s.items.map((item) =>
          item.id === service.id
            ? { ...item, isActive: active, activeCount: active ? item.totalListings : 0 }
            : item
        ),
      }));
      setDetail((s) => ({
        ...s,
        selected: s.selected?.id === service.id ? { ...s.selected, isActive: active } : s.selected,
      }));
    } catch (err) {
      setActivation((s) => ({
        ...s,
        errors: { ...s.errors, [service.id]: getErrorMessage(err, "Unable to update service status.") },
      }));
    } finally {
  ...
  const handleSavePrice = async (service, customPrice) => {
    if (customPrice !== null && (!Number.isFinite(customPrice) || customPrice <= 0)) {
      setPrice((s) => ({
        ...s,
        errors: { ...s.errors, [service.id]: "Enter a valid positive price, or clear to use global pricing." },
      }));
      return;
    }
    setPrice((s) => ({
    ...
    try {
      ...
      const updated = normalizeServiceDetailResponse({ data: [res.data] }, "getatext")[0];
      setDetail((s) => ({ ...s, entries: s.entries.map((e) => (e.id === service.id ? updated : e)) }));
      setPrice((s) => ({ ...s, statuses: { ...s.statuses, [service.id]: "saved" } }));
    } catch (err) {
      setPrice((s) => ({
        ...s,
        errors: { ...s.errors, [service.id]: getErrorMessage(err, "Unable to save custom price.") },
      }));
    } finally {
      ...
      <div className="flex items-start gap-3">
        ...
        <div>
          ...
          <p className="mt-1 text-sm text-gray-400">
            Manage GetAText USA number services — toggle activation and set custom prices.
          </p>
        </div>
      </div>
        ...
        <div className="flex items-center justify-between gap-3">
          {!list.loading && !list.error && list.items.length > 0 && (
            <p className="text-xs text-gray-400">
              <span className="font-semibold text-white">{list.items.length}</span>{" "}
              {list.items.length === 1 ? "service" : "services"} — click one to edit prices
            </p>
          )}
          {(list.loading || list.error || list.items.length === 0) && <span />}
          <button
            type="button"
