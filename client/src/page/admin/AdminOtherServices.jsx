...
const AdminOtherServices = () => {
  ...
  const loadServiceNames = async () => {
    ...
    try {
      ...
      setList({ items: normalizeServiceNameResponse(res, "smsbower"), loading: false, error: "" });
    } catch (err) {
      setList({ items: [], loading: false, error: getErrorMessage(err, "Unable to fetch other services.") });
    }
  };
  ...
  const loadServiceEntries = async (service) => {
    ...
    try {
      const res = await getSmsBowerServices({ page: 1, limit: 100, service: service.name });
      setDetail({
        selected: service,
  ...
  const handleSavePrice = async (service, customPrice) => {
    ...
    try {
      ...
      const updated = normalizeServiceDetailResponse({ data: [res.data] }, "smsbower")[0];
