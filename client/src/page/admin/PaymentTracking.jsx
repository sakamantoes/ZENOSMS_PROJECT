...
const PaymentTracking = () => {
  ...
  useEffect(() => {
    ...
    if (filterMethod !== 'ALL') {
      result = result.filter(deposit => deposit.paymentMethod === filterMethod);
    }
    ...
    setCurrentPage(1);
  }, [deposits, searchTerm, filterStatus, filterMethod, sortBy, sortOrder]);
  ...
  const getMethodBadge = (method) => {
    const methodMap = {
      'SQUAD': { label: 'Squad', className: 'bg-blue-500/10 text-blue-400 border-blue-500/20' },
      'MANUAL_TRANSFER': { label: 'Manual Transfer', className: 'bg-purple-500/10 text-purple-400 border-purple-500/20' },
      'QUEST': { label: 'Quest', className: 'bg-orange-500/10 text-orange-400 border-orange-500/20' }
    };
    return methodMap[method] || { label: method || 'Unknown', className: 'bg-gray-500/10 text-gray-400 border-gray-500/20' };
  };
  ...
  const DepositDetailsModal = ({ deposit, onClose }) => {
    ...
    const method = getMethodBadge(deposit.paymentMethod);
        ...
        >
          ...
          <div className="grid grid-cols-2 gap-4 mb-6">
            <div className="p-3 rounded-xl bg-white/5 border border-white/5">
              ...
              <p className="text-lg font-bold text-emerald-400">{formatCurrency(deposit.amount)}</p>
            </div>
            <div className="p-3 rounded-xl bg-white/5 border border-white/5">
            ...
            <div className="p-3 rounded-xl bg-white/5 border border-white/5">
              ...
              <span className={`inline-block px-2 py-1 rounded-full text-xs border mt-1 ${method.className}`}>
                {method.label}
              </span>
            </div>
            <div className="p-3 rounded-xl bg-white/5 border border-white/5">
              <p className="text-xs text-gray-400">Depositor</p>
              <p className="text-sm text-white font-medium truncate">{deposit.depositorName}</p>
            </div>
            ...
            {deposit.balanceBefore !== undefined && deposit.balanceAfter !== undefined && (
              <div className="p-3 rounded-xl bg-white/5 border border-white/5 col-span-2">
                <p className="text-xs text-gray-400">Balance Changes</p>
                <div className="flex items-center gap-3 mt-1">
                  <div>
                    <p className="text-xs text-gray-500">Before</p>
                    <p className="text-sm text-white font-medium">{formatCurrency(deposit.balanceBefore)}</p>
                  </div>
                  <ArrowUpRight className="w-4 h-4 text-emerald-400" />
                  <div>
                    <p className="text-xs text-gray-500">After</p>
                    <p className="text-sm text-emerald-400 font-medium">{formatCurrency(deposit.balanceAfter)}</p>
                  </div>
                </div>
              </div>
            )}
            ...
            <table className="w-full">
              <thead className="bg-white/5 border-b border-white/5">
                <tr>
                  ...
                  <th className="text-left p-4 text-xs font-medium text-gray-400 uppercase tracking-wider">Method</th>
                  ...
                  currentItems.map((deposit, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        <td className="p-4">
                          <span className={`px-2 py-1 rounded-full text-xs border inline-flex items-center gap-1 ${method.className}`}>
                            {method.label}
                          </span>
                        </td>
                        <td className="p-4">
